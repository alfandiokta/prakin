...
class Pendaftar extends CI_Controller {
	...
	public function index()
	{
		...
		$data['magang']=$this->pendaftar->getmagang();
		$data['kuota']=$this->pendaftar->kuota();
	...
	public function update_data_ditolak($id)
	{
	...
	public function detail($id)
	{
		...
		$data['user'] = $this->db->get_where('tbl_user',['email' => 
		$this->session->userdata('email')])->row_array();
		$data['get1'] = $this->pendaftar->getdata($id);
		
		$data['get2'] = $this->pendaftar->userId($id);
		// var_dump($data['get1']);
		// die;
    ...
    $this->load->helper(array('download','url'));
    force_download(FCPATH.'./assets/doc/tgs/'.$tgs, null);
	redirect('pendaftar/detail');
	}
	public function wa()
	{
